  ...
  return (
    <button onClick={onChange} disabled={loading} style={{
      ...
      <div style={{
        ...
        background: '#0f0f1a', transition: 'left 0.2s',
...
function AdminFlags() {
  ...
  const inp = { width: '100%', padding: '8px 12px', border: '1px solid #1e1e2e', borderRadius: '8px', fontSize: '13px', fontFamily: 'inherit', outline: 'none', color: '#e2e8f0', background: '#0f0f1a' };
      ...
      <AdminLayout title="Feature Flags" breadcrumbs={['Feature Flags']}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', background: '#0f0f1a', border: '1px solid #1e1e2e', borderRadius: '8px' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search flags..." style={{ border: 'none', outline: 'none', fontSize: '13px', color: '#9999bb', fontFamily: 'inherit', width: '160px' }} />
            </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '14px' }}>
            {filtered.map(flag => (
              <div key={flag.id} style={{ background: '#0f0f1a', border: `1px solid ${flag.enabled ? '#1e1e2e' : '#111122'}`, borderRadius: '12px', padding: '18px', opacity: flag.enabled ? 1 : 0.7, transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                      <span style={{ fontSize: '14px', fontWeight: '600', color: '#e2e8f0' }}>{flag.label}</span>
                ...
                {flag.description && <p style={{ fontSize: '12px', color: '#6b6b8a', marginBottom: '12px', lineHeight: '1.5' }}>{flag.description}</p>}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button onClick={() => handleDelete(flag)}
                      style={{ padding: '4px 10px', background: '#2d0a0a', border: '1px solid #fecaca', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', color: '#dc2626' }}>Delete</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {showModal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '20px' }} onClick={() => setShowModal(false)}>
            <div style={{ background: '#0f0f1a', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '480px' }} onClick={e => e.stopPropagation()}>
              <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#e2e8f0', marginBottom: '20px' }}>{editFlag ? 'Edit Flag' : 'New Feature Flag'}</h3>
              <div style={{ display: 'grid', gap: '12px' }}>
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#9999bb', display: 'block', marginBottom: '5px' }}>Key *</label>
                ...
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#9999bb', display: 'block', marginBottom: '5px' }}>Label *</label>
                  <input style={inp} value={form.label} onChange={e => setForm({...form, label: e.target.value})} placeholder="My Feature Flag" /></div>
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#9999bb', display: 'block', marginBottom: '5px' }}>Description</label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#9999bb', display: 'block', marginBottom: '5px' }}>Environment</label>
                  ...
                  <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#9999bb', display: 'block', marginBottom: '5px' }}>Rollout %</label>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontSize: '13px', color: '#9999bb' }}>{form.enabled ? 'Enabled' : 'Disabled'}</span>
                </div>
              </div>
