...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (n8nResult.success && n8nResult.data?.code) {
      const response = { code: n8nResult.data.code, code_type, model: n8nResult.data.model || 'openrouter', prompt: prompt.trim() };
    ...
    // ── Step 2: Fallback → direct AI ──────────────────────
    try {
      const systemPrompt = buildSystemPrompt(code_type, config);
      const result = await callAI({
        messages: [
          { role: 'system', content: systemPrompt },
          { role: 'user', content: `Generate a ServiceNow ${CODE_TYPES[code_type].label} that does:\n\n${prompt.trim()}\n\nReturn ONLY the code, no markdown backticks.` },
        ],
      });
      const response = { code: result.content, code_type, model: result.model, prompt: prompt.trim() };
      await cacheSet(cacheKey, JSON.stringify(response), 3600);
      query('INSERT INTO sn_code_generations (user_id,code_type,prompt,generated,model) VALUES ($1,$2,$3,$4,$5)',
        [user_id || null, code_type, prompt.trim().substring(0, 500), result.content, result.model]).catch(() => {});
      return res.status(200).json({ success: true, ...response, via: 'direct' });
    } catch (err) {
