...
async function handler(req, res) {
  ...
  try {
    const result = await callAI({
      messages: [
        { role: 'system', content: 'Expert full-stack developer debugging a ServiceNow SaaS platform. Stack: Next.js, PostgreSQL, Redis, n8n, Docker. Give concise, actionable advice.' },
        { role: 'user', content: contextData ? `Context:\n${contextData}\n\nQuestion: ${question}` : question },
      ],
    });
    return res.status(200).json({ success: true, answer: result.content, model: result.model, context_used: context_type, via: 'direct' });
  } catch (err) {
