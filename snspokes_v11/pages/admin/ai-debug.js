        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px', alignItems: 'start' }}>
          ...
          <div>
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>Context</p>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {CONTEXT_TYPES.map(c => (
                  <button key={c.value} onClick={() => setContextType(c.value)} style={{ padding: '6px 14px', borderRadius: '8px', border: '1px solid', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px', fontWeight: '500', background: contextType === c.value ? '#111827' : '#f9fafb', borderColor: contextType === c.value ? '#111827' : '#e5e7eb', color: contextType === c.value ? '#fff' : '#374151', transition: 'all 0.15s' }} title={c.desc}>
                    {c.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px', marginBottom: '16px' }}>
              <textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="Ask anything about your system — debug errors, optimize queries, generate code..." rows={4}
                ...
                style={{ width: '100%', padding: '10px 12px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', outline: 'none', resize: 'vertical', color: '#111827' }} />
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                <span style={{ fontSize: '11px', color: '#9ca3af' }}>⌘+Enter to send</span>
                <button onClick={handleAsk} disabled={loading || !question.trim()} style={{ padding: '8px 20px', background: loading || !question.trim() ? '#9ca3af' : '#111827', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: loading || !question.trim() ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            {/* Error */}
            {error && <div style={{ padding: '12px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', color: '#dc2626', fontSize: '13px', marginBottom: '16px' }}>⚠️ {error}</div>}
              ...
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '13px', fontWeight: '600', color: '#111827' }}>AI Response</span>
                  </div>
                  <span style={{ fontSize: '11px', color: '#9ca3af' }}>{answer.model}</span>
                </div>
                <div style={{ padding: '16px', fontSize: '14px', color: '#374151', lineHeight: '1.7', whiteSpace: 'pre-wrap' }}>
                  {answer.answer}
                </div>
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6' }}>
                <p style={{ fontSize: '12px', fontWeight: '600', color: '#374151', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Quick Prompts</p>
              </div>
              <div style={{ padding: '8px' }}>
                {QUICK_PROMPTS.map((p, i) => (
                  <button key={i} onClick={() => setQuestion(p)} style={{ width: '100%', textAlign: 'left', padding: '8px 10px', background: 'none', border: 'none', borderRadius: '6px', fontSize: '12px', color: '#6b7280', cursor: 'pointer', fontFamily: 'inherit', lineHeight: '1.5', transition: 'all 0.1s' }}
            ...
            {history.length > 0 && (
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between' }}>
                  <p style={{ fontSize: '12px', fontWeight: '600', color: '#374151', textTransform: 'uppercase', letterSpacing: '0.5px' }}>History</p>
                ...
                <div style={{ padding: '8px' }}>
                  {history.map((h, i) => (
                    <button key={i} onClick={() => { setQuestion(h.question); setContextType(h.context); setAnswer({ answer: h.answer, model: h.model }); }}
                      style={{ width: '100%', textAlign: 'left', padding: '8px 10px', background: 'none', border: 'none', borderRadius: '6px', fontSize: '12px', color: '#6b7280', cursor: 'pointer', fontFamily: 'inherit', lineHeight: '1.5', transition: 'all 0.1s', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                      onMouseEnter={e => { e.currentTarget.style.background = '#f9fafb'; e.currentTarget.style.color = '#111827'; }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = '#6b7280'; }}
                    >💬 {h.question}</button>
                  ))}
