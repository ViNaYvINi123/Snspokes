          ...
          <>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'14px', marginBottom:'20px' }}>
              {[
                ...
                <div key={s.label} style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:'10px', padding:'16px' }}>
            ...
            <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:'10px', padding:'16px', marginBottom:'20px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:'13px', marginBottom:'8px' }}>
                <span style={{ fontWeight:'600', color:'#374151' }}>Content Completeness</span>
                  ...
                  {(data?.[tab==='missing'?'missing_content':tab==='low_rated'?'low_rated':'stale_content']||[]).map(spoke => (
                    <tr key={spoke.id} style={{ borderBottom:'1px solid #f9fafb' }}>
                      <td style={{ padding:'10px 14px' }}>
                        <div style={{ fontWeight:'600', color:'#111827' }}>{spoke.name}</div>
                      ...
                      <td style={{ padding:'10px 14px', textAlign:'center' }}>
                        <div style={{ display:'flex', gap:'6px', justifyContent:'center' }}>
                          ...
                          <button onClick={()=>verify(spoke.id)} style={{ padding:'4px 10px', background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:'6px', fontSize:'11px', color:'#16a34a', cursor:'pointer', fontFamily:'inherit', fontWeight:'600' }}>✓ Verify</button>
                        </div>
                      </td>
                    </tr>
                  ))}
