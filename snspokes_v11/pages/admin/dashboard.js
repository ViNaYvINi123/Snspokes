  ...
  return (
    <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '20px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
        ...
        {trend !== undefined && (
          <span style={{ fontSize: '12px', padding: '2px 8px', borderRadius: '20px', background: trend >= 0 ? '#f0fdf4' : '#fef2f2', color: trend >= 0 ? '#16a34a' : '#dc2626', fontWeight: '600', border: `1px solid ${trend >= 0 ? '#bbf7d0' : '#fecaca'}` }}>
            {trend >= 0 ? '↑' : '↓'} {Math.abs(trend)}%
          </span>
        )}
      </div>
      <div style={{ fontSize: '26px', fontWeight: '700', color: '#111827', letterSpacing: '-0.5px', marginBottom: '4px' }}>{value}</div>
...
function StatusPill({ ok, label }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '6px 12px', background: ok ? '#f0fdf4' : '#fef2f2', border: `1px solid ${ok ? '#bbf7d0' : '#fecaca'}`, borderRadius: '8px' }}>
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '24px', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <button onClick={fetchData} style={{ padding: '6px 14px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', color: '#374151', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '500' }}
                ...
                {redis.connected ? (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    {[
                      ...
                      <div key={s.label} style={{ padding: '12px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #f3f4f6' }}>
                        <div style={{ fontSize: '18px', fontWeight: '700', color: '#111827' }}>{s.val}</div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px' }}>
                      {[
                        ...
                        <div key={s.label} style={{ padding: '10px 8px', background: '#f9fafb', borderRadius: '8px', textAlign: 'center', border: '1px solid #f3f4f6' }}>
                  ...
                  return (
                    <div key={i} style={{ marginBottom: '12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                        <span style={{ fontSize: '13px', color: '#374151', fontWeight: '500' }}>{t.query}</span>
                ...
                {(data?.recent_queries || []).length > 0
                  ? (data.recent_queries || []).slice(0, 8).map((q, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < 7 ? '1px solid #f9fafb' : 'none' }}>
                      <span style={{ fontSize: '13px', color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '200px' }}>{q.query}</span>
