...
function Card({ children, style={} }) {
  return <div style={{ background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',overflow:'hidden',...style }}>{children}</div>;
}
...
function Toast({ msg, onClose }) {
  ...
  return (
    <div style={{ position:'fixed',bottom:'24px',right:'24px',zIndex:9999,padding:'12px 18px',borderRadius:'10px',background: ok ? '#f0fdf4' : '#fef2f2',border:`1px solid ${ok ? '#bbf7d0' : '#fecaca'}`,color: ok ? '#16a34a' : '#dc2626',fontSize:'13px',fontWeight:'500',boxShadow:'0 4px 12px rgba(0,0,0,0.1)' }}>
      {ok ? '✅' : '⚠️'} {msg.text}
    </div>
  );
}
...
export default function APIConnectors() {
  ...
  const inp = { width:'100%',padding:'8px 12px',border:'1px solid #e5e7eb',borderRadius:'7px',fontSize:'13px',fontFamily:'inherit',outline:'none',color:'#111827',background:'#fff' };
  ...
  return (
    <>
      ...
      <AdminLayout title="API Integration Platform" breadcrumbs={['API Connectors']}>
        ...
        {/* View Tabs */}
        <div style={{ display:'flex',gap:'0',marginBottom:'20px',background:'#fff',border:'1px solid #e5e7eb',borderRadius:'10px',padding:'4px',width:'fit-content' }}>
          {[['list','🔌 Connectors'],['webhooks','📡 Webhooks']].map(([v,l]) => (
            <button key={v} onClick={() => { setView(v); if (v==='webhooks') fetchWebhooks(); }}
              style={{ padding:'7px 16px',borderRadius:'7px',border:'none',fontSize:'13px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit',background: view===v ? '#111827' : 'transparent',color: view===v ? '#fff' : '#6b7280',transition:'all 0.12s' }}>{l}</button>
          ))}
        </div>
                  ...
                  <div>
                    {connectors.map(c => (
                      <div key={c.id} onClick={() => { setSelected(c); fetchConnectorDetail(c.id); setView('tester'); }}
                        style={{ padding:'12px 16px',borderBottom:'1px solid #f9fafb',cursor:'pointer',display:'flex',alignItems:'center',gap:'10px',background: selected?.id===c.id ? '#f9fafb' : 'transparent',transition:'background 0.1s' }}
                        onMouseEnter={e => e.currentTarget.style.background='#f9fafb'}
                        onMouseLeave={e => e.currentTarget.style.background= selected?.id===c.id ? '#f9fafb' : 'transparent'}
                      >
            ...
            {selected && (
              <div style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
                ...
                <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                  <div>
                    <h2 style={{ fontSize:'16px',fontWeight:'700',color:'#111827',marginBottom:'4px' }}>{selected.name}</h2>
                ...
                <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px' }}>
                  {[
                    ...
                    <Card key={s.l} style={{ padding:'14px' }}>
                      <div style={{ fontSize:'20px',fontWeight:'700',color:'#111827',marginBottom:'3px' }}>{s.v}</div>
                ...
                <Card>
                  ...
                  {testResult && (
                    <div style={{ margin:'0 16px 16px',borderRadius:'8px',border:'1px solid #e5e7eb',overflow:'hidden' }}>
                      <div style={{ padding:'8px 12px',background: testResult.success ? '#f0fdf4' : '#fef2f2',borderBottom:'1px solid #e5e7eb',display:'flex',gap:'12px',fontSize:'12px',fontWeight:'600' }}>
                      ...
                      <pre style={{ padding:'12px',fontFamily:'monospace',fontSize:'12px',color:'#374151',overflow:'auto',maxHeight:'240px',margin:0,background:'#f9fafb' }}>
                ...
                <Card>
                  ...
                  {!editEp && epForm !== EMPTY_ENDPOINT || true ? (
                    <div style={{ padding:'12px 16px',borderBottom:'1px solid #f3f4f6',display:'grid',gridTemplateColumns:'80px 1fr auto auto',gap:'8px',alignItems:'end' }}>
                      <div>
                        <label style={lbl}>Method</label>
                  ...
                  {endpoints.map(ep => (
                    <div key={ep.id} style={{ padding:'10px 16px',borderBottom:'1px solid #f9fafb',display:'flex',alignItems:'center',gap:'10px' }}>
                      ...
                      <code style={{ fontSize:'13px',color:'#374151',flex:1 }}>{ep.path}</code>
                ...
                <tbody>
                  {webhooks.map(wh => (
                    <tr key={wh.id} style={{ borderBottom:'1px solid #f9fafb' }}>
                      <td style={{ padding:'12px 16px',fontWeight:'600',color:'#111827' }}>{wh.name}</td>
