...
export default function EnvManager() {
  ...
  const inp = { width: '100%', padding: '7px 10px', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '13px', fontFamily: 'monospace', outline: 'none', background: '#f9fafb' };
    ...
    <>
      ...
      <AdminLayout title="Environment Manager" breadcrumbs={['System', 'Env Manager']}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
          ...
          <button onClick={() => setShowAdd(true)} style={{ padding: '8px 16px', background: '#111827', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>+ Add Variable</button>
        </div>
        ...
        {loading ? <div style={{ textAlign: 'center', padding: '60px', color: '#9ca3af' }}>Loading...</div> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {Object.entries(CATEGORIES).filter(([, items]) => items.length > 0).map(([cat, items]) => (
              <div key={cat} style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', background: '#f9fafb', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '11px', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.8px' }}>{cat}</span>
                    ...
                    {items.filter(v => !search || v.key.toLowerCase().includes(search.toLowerCase())).map((v, i) => (
                      <tr key={v.key} style={{ borderBottom: i < items.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                        <td style={{ padding: '12px 16px', width: '40%' }}>
                          <code style={{ fontFamily: 'monospace', fontSize: '12px', color: '#374151', fontWeight: '600' }}>{v.key}</code>
                        </td>
                        <td style={{ padding: '12px 16px' }}>
                          {editing === v.key ? (
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <code style={{ fontFamily: 'monospace', fontSize: '12px', color: v.sensitive && !revealed[v.key] ? '#9ca3af' : '#374151', background: '#f9fafb', padding: '3px 8px', borderRadius: '4px', border: '1px solid #e5e7eb', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '300px' }}>
                        ...
                        <td style={{ padding: '12px 16px', width: '160px' }}>
                          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
                            {editing === v.key ? (
                              <>
                                <button onClick={() => handleSave(v.key)} disabled={saving} style={{ padding: '4px 12px', background: '#111827', border: 'none', borderRadius: '6px', color: '#fff', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>
                                <button onClick={() => setEditing(null)} style={{ padding: '4px 10px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '12px', cursor: 'pointer' }}>✕</button>
                              </>
                            ) : (
                              <>
                                ...
                                <button onClick={() => handleDelete(v.key)}
                                  style={{ padding: '4px 10px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', color: '#dc2626' }}>Del</button>
                              </>
                            )}
                          </div>
                        </td>
        ...
        {showAdd && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={() => setShowAdd(false)}>
            <div style={{ background: '#fff', borderRadius: '14px', padding: '24px', width: '440px', maxWidth: '95vw' }} onClick={e => e.stopPropagation()}>
              <h3 style={{ fontSize: '15px', fontWeight: '700', color: '#111827', marginBottom: '18px' }}>Add Environment Variable</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#374151', display: 'block', marginBottom: '4px' }}>Variable Name *</label>
                ...
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#374151', display: 'block', marginBottom: '4px' }}>Value</label>
                ...
                <div><label style={{ fontSize: '12px', fontWeight: '500', color: '#374151', display: 'block', marginBottom: '4px' }}>Comment (optional)</label>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '18px' }}>
                <button onClick={() => setShowAdd(false)} style={{ flex: 1, padding: '9px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px' }}>Cancel</button>
                <button onClick={handleAdd} disabled={saving} style={{ flex: 2, padding: '9px', background: '#111827', border: 'none', borderRadius: '8px', color: '#fff', cursor: 'pointer', fontFamily: 'inherit', fontSize: '13px', fontWeight: '600' }}>Add Variable</button>
              </div>
            </div>
          </div>
        )}
