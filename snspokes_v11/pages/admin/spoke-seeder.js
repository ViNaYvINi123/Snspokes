...
export default function SpokeSeeder() {
      ...
      <AdminLayout title="Spoke Content Seeder" breadcrumbs={['Catalog', 'Spoke Seeder']}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#111827', marginBottom: '16px' }}>📊 Enrichment Status</h3>
          ...
          {/* Batch control */}
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#111827', marginBottom: '16px' }}>⚡ Batch Enrichment</h3>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '14px' }}>
              <label style={{ fontSize: '12px', color: '#374151', fontWeight: '500', whiteSpace: 'nowrap' }}>Batch size:</label>
              <select value={batchSize} onChange={e => setBatchSize(parseInt(e.target.value))}
                style={{ padding: '6px 10px', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '13px', fontFamily: 'inherit', outline: 'none' }}>
                {[1, 2, 3, 5, 10].map(n => <option key={n} value={n}>{n} spoke{n > 1 ? 's' : ''} (~{n * 15}s)</option>)}
              </select>
            </div>
            <button onClick={enrichBatch} disabled={running || needsWork === 0}
              style={{ width: '100%', padding: '10px', background: running || needsWork === 0 ? '#9ca3af' : '#111827', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: running || needsWork === 0 ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
        ...
        {pending.length > 0 && (
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden', marginBottom: '20px' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: '13px', fontWeight: '600', color: '#111827', margin: 0 }}>📋 Needs Enrichment (showing {pending.length})</h3>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  <th style={{ padding: '9px 14px', textAlign: 'left', fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Spoke</th>
                  <th style={{ padding: '9px 14px', textAlign: 'left', fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Category</th>
                  <th style={{ padding: '9px 14px', textAlign: 'right', fontSize: '11px', fontWeight: '600', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {pending.map(sp => (
