...
export default function AdminAnnouncements() {
  ...
  const inp = { width:'100%', padding:'8px 10px', border:'1px solid #e5e7eb', borderRadius:'7px', fontSize:'13px', fontFamily:'inherit', outline:'none' };
      ...
      <AdminLayout title="Announcement System" breadcrumbs={['Announcements']}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px' }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:'12px', overflow:'hidden' }}>
            <div style={{ padding:'12px 16px', borderBottom:'1px solid #f3f4f6' }}><h3 style={{ fontSize:'13px', fontWeight:'600', color:'#111827', margin:0 }}>📢 New Announcement</h3></div>
            <div style={{ padding:'16px', display:'flex', flexDirection:'column', gap:'12px' }}>
              ...
              <button onClick={save} disabled={saving} style={{ padding:'10px', background:'#111827', border:'none', borderRadius:'8px', color:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', fontFamily:'inherit' }}>
          ...
          <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:'12px', overflow:'hidden' }}>
            <div style={{ padding:'12px 16px', borderBottom:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <h3 style={{ fontSize:'13px', fontWeight:'600', color:'#111827', margin:0 }}>Active Announcements ({announcements.length})</h3>
            </div>
            <div>
                ...
                <div key={a.id} style={{ padding:'14px 16px', borderBottom:'1px solid #f9fafb', opacity: a.is_active ? 1 : 0.5 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px' }}>
                    ...
                    <div style={{ display:'flex', gap:'6px', flexShrink:0, marginLeft:'10px' }}>
                      <button onClick={()=>toggle(a.id,a.is_active)} style={{ padding:'4px 10px', background: a.is_active ? '#fef2f2' : '#f0fdf4', border:`1px solid ${a.is_active ? '#fecaca' : '#bbf7d0'}`, borderRadius:'6px', fontSize:'11px', cursor:'pointer', color: a.is_active ? '#dc2626' : '#16a34a', fontFamily:'inherit', fontWeight:'600' }}>
                        {a.is_active ? 'Disable' : 'Enable'}
                      </button>
                      <button onClick={()=>del(a.id)} style={{ padding:'4px 8px', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:'6px', fontSize:'11px', cursor:'pointer', color:'#dc2626' }}>×</button>
                    </div>
                  </div>
        ...
        {toast && <div style={{ position:'fixed', bottom:'24px', right:'24px', zIndex:9999, padding:'12px 18px', borderRadius:'10px', background: toast.type==='error' ? '#fef2f2' : '#f0fdf4', border:`1px solid ${toast.type==='error' ? '#fecaca' : '#bbf7d0'}`, color: toast.type==='error' ? '#dc2626' : '#16a34a', fontSize:'13px', fontWeight:'500', boxShadow:'0 4px 12px rgba(0,0,0,0.1)' }}>{toast.type==='error'?'⚠️':'✅'} {toast.text}</div>}
      </AdminLayout>
    </>
  );
}
