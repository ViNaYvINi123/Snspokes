...
export default function ScriptLinter() {
  ...
  const lint = async () => {
    ...
    try {
      ...
      setResult(res.data);
    } catch (err) { alert(err.response?.data?.error || 'Lint failed'); }
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 16px' }}>
          <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '6px' }}>
              ...
              <h1 style={{ fontSize: '22px', fontWeight: '800', color: '#111827', letterSpacing: '-0.5px' }}>Script Linter & Analyzer</h1>
            </div>
            <p style={{ fontSize: '14px', color: '#6b7280' }}>Paste any ServiceNow script. Get instant detection of bugs, performance issues, and security vulnerabilities.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '16px', alignItems: 'start' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
                <select value={scriptType} onChange={e => setScriptType(e.target.value)} style={{ padding: '7px 12px', border: '1px solid #e5e7eb', borderRadius: '7px', fontSize: '13px', fontFamily: 'inherit', outline: 'none', background: '#fff' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
                  <button onClick={() => setScript(SAMPLE_SCRIPTS.good)} style={{ padding: '5px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '6px', fontSize: '12px', color: '#16a34a', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '600' }}>Load Good Example</button>
                  <button onClick={() => setScript(SAMPLE_SCRIPTS.bad)} style={{ padding: '5px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '6px', fontSize: '12px', color: '#dc2626', cursor: 'pointer', fontFamily: 'inherit', fontWeight: '600' }}>Load Bad Example</button>
                </div>
              </div>
              ...
              <div style={{ position: 'relative', marginBottom: '12px' }}>
                <textarea value={script} onChange={e => setScript(e.target.value)} rows={16}
                  ...
                  style={{ width: '100%', padding: '14px', border: '1px solid #e5e7eb', borderRadius: '10px', fontSize: '12px', fontFamily: 'JetBrains Mono, monospace', resize: 'vertical', outline: 'none', color: '#e2e8f0', background: '#0f172a', lineHeight: '1.7' }} />
              ...
              {result && (
                <div>
                  {/* Score header */}
                  <div style={{ display: 'flex', gap: '14px', alignItems: 'center', padding: '16px 20px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', marginBottom: '12px' }}>
                    <div style={{ width: '60px', height: '60px', borderRadius: '12px', background: GRADE_BG[result.grade], border: `2px solid ${GRADE_COLOR[result.grade]}30`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  {result.issues.map((issue, i) => (
                    <div key={i} style={{ padding: '12px 16px', marginBottom: '8px', borderRadius: '10px', background: SEV_BG[issue.severity], border: `1px solid ${SEV_BORDER[issue.severity]}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px' }}>
                        ...
                        <span style={{ fontSize: '11px', color: '#6b7280', fontWeight: '600' }}>{issue.category}</span>
                      </div>
                      <p style={{ fontSize: '13px', fontWeight: '600', color: '#111827', margin: '0 0 5px' }}>{issue.message}</p>
                      <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>💡 {issue.fix}</p>
                    </div>
                  ))}

                  {/* AI Review */}
                  {result.ai_review && (
                    <div style={{ padding: '16px', background: '#f5f3ff', border: '1px solid #c4b5fd', borderRadius: '10px', marginTop: '12px' }}>
                      <div style={{ fontSize: '13px', fontWeight: '700', color: '#6c63ff', marginBottom: '10px' }}>🤖 AI Deep Review</div>
                      <p style={{ fontSize: '13px', color: '#374151', lineHeight: '1.6', marginBottom: '10px' }}>{result.ai_review.summary}</p>
                      {result.ai_review.critical_issues?.map((i, idx) => (
                        <div key={idx} style={{ padding: '6px 10px', background: '#fef2f2', borderRadius: '6px', fontSize: '12px', color: '#dc2626', marginBottom: '5px' }}>❌ {i}</div>
                      ))}
                      {result.ai_review.suggestions?.map((s, idx) => (
                        <div key={idx} style={{ padding: '6px 10px', background: '#fff', borderRadius: '6px', fontSize: '12px', color: '#374151', marginBottom: '5px', border: '1px solid #e5e7eb' }}>💡 {s}</div>
                      ))}
            ...
            <div>
              <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
                <div style={{ padding: '12px 14px', borderBottom: '1px solid #f3f4f6' }}>
                  <span style={{ fontSize: '12px', fontWeight: '700', color: '#374151', textTransform: 'uppercase', letterSpacing: '0.5px' }}>📋 What We Check</span>
                </div>
                <div style={{ padding: '12px 14px' }}>
                  {[
                    ...
                    <div key={section.cat} style={{ marginBottom: '14px' }}>
                      <div style={{ fontSize: '12px', fontWeight: '700', color: '#374151', marginBottom: '6px' }}>{section.icon} {section.cat}</div>
                      {section.checks.map(c => (
                        <div key={c} style={{ fontSize: '11px', color: '#6b7280', padding: '2px 0', paddingLeft: '12px' }}>• {c}</div>
                      ))}
                    </div>
                  ))}
                </div>
