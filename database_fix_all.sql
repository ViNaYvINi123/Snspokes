...
ALTER TABLE sn_users ADD COLUMN IF NOT EXISTS onboarded_at TIMESTAMP;
...
ALTER TABLE sn_spokes ADD COLUMN IF NOT EXISTS avg_rating NUMERIC(3,2) DEFAULT 0;
ALTER TABLE sn_spokes ADD COLUMN IF NOT EXISTS rating_count INT DEFAULT 0;
ALTER TABLE sn_spokes ADD COLUMN IF NOT EXISTS code_examples JSONB;
...
-- Extended error encyclopedia (zero AI — instant lookup)
INSERT INTO sn_error_encyclopedia (error_pattern, title, description, root_cause, fix_steps, category, severity) VALUES
('JavaScript heap out of memory', 'Node Heap Memory Exhaustion', 'Script uses too much memory, causing process crash', 'Unbounded array growth, loading entire tables into memory, or recursive data structures', '["Use setLimit() on all GlideRecord queries","Process records in batches of 100-500","Use GlideAggregate instead of loading all records","Add --max-old-space-size flag if running in MID server"]', 'Performance', 'critical'),
('Could not find function', 'Function Not Found Error', 'Script calls a function that does not exist in current scope', 'Typo in function name, wrong script scope, or function defined in different scope', '["Check function name spelling","Verify function is in correct Script Include","Use GlideScriptEvaluator to check scope","Check if function is client-side vs server-side"]', 'Scripting', 'medium'),
('GlideRecord not found', 'Table Does Not Exist', 'GlideRecord cannot find the specified table', 'Wrong table name, table in different scope, or table not yet created', '["Verify table name in sys_db_object","Check application scope prefix","Confirm table exists in target instance","Try sys_db_object.list to browse tables"]', 'Scripting', 'low'),
('Blocked by security constraint', 'Security Policy Blocked', 'IP address or action blocked by instance security policy', 'IP not in allowlist, too many failed logins, or security plugin restriction', '["Check glide.security properties","Review IP Access Control Lists","Check if IP is blocked in sys_security_group","Contact instance admin to whitelist IP"]', 'Security', 'high'),
('Timeout waiting for', 'Connection Timeout', 'Outbound integration timed out waiting for response', 'Target system is slow, network issue, or firewall blocking connection', '["Increase timeout in Connection & Credential alias","Check target system health","Verify network/firewall rules on MID server","Try testing connectivity via MID server SSH"]', 'Integration', 'medium'),
('Import set transform error', 'Transform Map Failure', 'Data import transform encountered an error during processing', 'Missing required field mapping, data type mismatch, or transform script error', '["Check transform map field mappings","Verify source data matches expected format","Test with single record first","Check transform scripts for null handling"]', 'Development', 'medium'),
('Quota exceeded', 'API Rate Limit / Quota Error', 'REST API call rejected due to rate limit or quota restriction', 'Too many API calls per timeframe, or account-level quota reached', '["Implement exponential backoff retry","Check rate limit headers in response","Use bulk APIs instead of individual calls","Cache responses to reduce API calls"]', 'Integration', 'medium'),
('Update conflict', 'Record Update Conflict', 'Two users or processes tried to update the same record simultaneously', 'Concurrent modification, missing setWorkflow(false), or slow business rules causing race condition', '["Add record locking with GlideLock","Use setWorkflow(false) for programmatic updates","Implement optimistic concurrency with sys_mod_count","Reduce business rule execution time"]', 'Scripting', 'medium')
ON CONFLICT (error_pattern) DO NOTHING;
